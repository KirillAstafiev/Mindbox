    ...
    foreach ($urls as $city => $url) {
        ...
        try {
            $conn->exec("UPDATE ZN_EventData
                            SET ЗаказНарядСтатус = 
                                CASE
                                    WHEN ЗаказНарядФактическаяДатаВыдачи IS NULL 
                                        AND ЗаказНарядФактическоеВремяВыдачи IS NULL
                                        AND ЗаказНарядПлановаяДатаВыдачи IS NOT NULL
                                        AND ЗаказНарядПлановоеВремяВыдачи IS NOT NULL THEN 'Готово к выдаче'
                                    WHEN ЗаказНарядФактическаяДатаВыдачи IS NULL 
                                        AND ЗаказНарядФактическоеВремяВыдачи IS NULL THEN 'В работе'
                                    WHEN ЗаказНарядФактическаяДатаВыдачи IS NOT NULL 
                                        AND ЗаказНарядФактическоеВремяВыдачи IS NOT NULL THEN 'Выдано'
                                    ELSE 'Завершено'
                                END;");
